{
  		...
  		this.show = function(auctionList, remainingTime, articleMap, offerMap) {
			...
			self.createTable(self.table, self.mode, auctionList);
  			auctionList.forEach(function(auction) {
				...
				if (offerMap[id_auction] === undefined) {
	...
	function OfferPage(tbody) {
		this.tbody = tbody;//tbody h4 tr th (tr td)* tr td p td form input button

		this.show = function(offerList) {
			console.log("lista offerte: ");
	...
	function OfferForm(tbody, offerPage) {
		...
		this.show = function(maxOffer, auction) {
			console.log("massima offerta: ");
			console.log(maxOffer);
			let form = document.createElement("form");
			let row, formTD;
			let input, p, actionButton, closeButton, hiddenAuctionID;
			let self = this;
			...
			hiddenAuctionID = document.createElement("input");
			hiddenAuctionID.type = "text";
			hiddenAuctionID.value = auction.id_asta;
			hiddenAuctionID.hidden = "true";
			hiddenAuctionID.name = "id_asta";
			
			form.appendChild(input);
			form.appendChild(actionButton);
			form.appendChild(closeButton);
			form.appendChild(hiddenAuctionID);
	...
	// TODO session-cookie logic -> update cookies
}
