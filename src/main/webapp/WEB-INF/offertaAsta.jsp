    ...
    <body>
        <div id="auctionOffer">
        	...
        	<c:if test="${not empty auction }" >
        		...
        		<br>
        		<c:set var="auctionID" value="${auction.id_asta }" scope="session"/>
        	</c:if>
        	<c:choose>
	        ...
	        <form action="MakeAnOffer" method="post">
	        	...
	        	<c:if test="${not empty sessionScope.offertaError }">
	        		<p style="color : red;">${sessionScope.offertaError }</p>
	        		<c:remove var="offertaError" scope="session"/>
	        	</c:if>
	        </form>
        </div>
    </body>
